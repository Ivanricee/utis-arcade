...
import { generateZeppInstances } from '../../../store/zepp-data'
import { ZeppRigidBody } from './ZeppRigidBody'
...
export function ZeppField() {
  const { geometries, material } = usePlasticMeshes()
  const instancedMeshRef = useRef<THREE.InstancedMesh>(null)
  const rigidBodyRefs = useRef<(RapierRigidBody | null)[]>([])

  const instances = useMemo(() => generateZeppInstances({ left: 0, central: 1, right: 0 }), [])
  const totalZepps = instances.length
  const pivotOffset = useMemo(() => new THREE.Vector3(0.28, -1.4433, 0.08), [])
